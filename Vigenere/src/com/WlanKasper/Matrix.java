    ...
    public String encryption(String phrase, String key) {
        ...
        if (phrase != null && key != null){
            ...
            for (int index = 0; index < phrase.length(); index++) {
                int indexPhrase = ALPH.indexOf(phrase.substring(index, (index + 1)));
                int indexKey = ALPH.indexOf(key.substring(index % key.length(), index % key.length() + 1));
                encryptedPhrase[index] = matrix[indexPhrase][indexKey];
            }
    ...
    public String decryption(String phrase, String key) {
        ...
        if (phrase != null && key != null){
            ...
            for (int index = 0; index < phrase.length(); index++) {
                int indexPhrase = ALPH.indexOf(phrase.substring(index, (index + 1)));
                int indexKey = ALPH.indexOf(key.substring(index % key.length(), index % key.length() + 1));
                decryptedPhrase[index] = matrix[(indexPhrase - indexKey) < 0 ? ALPH.length() + (indexPhrase - indexKey) : indexPhrase - indexKey ][0];
            }
